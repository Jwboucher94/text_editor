...
menu_count = "1-"+str(len(menu_options));
...
def pickDirectory(path_local, parent_local):
    ...
    folder = userText('Enter name of Directory (Use ".." to go back):\n>> ');
...
def fileNaming():
    ...
    print("");
    file_name = str(userText("Please enter file name: \n>> "))
...
def finished():
    #response = userText("Are you finished? (y/n)\n>> ");
    if "y" in userText("Are you finished? (y/n)\n>> "):
        exit();
    else:
        return;
...
def question():
    os.system(clear);
    print("Choose One " + menu_count + ":");
    for arg in menu_options:
        print (arg);
    response = userText(">> ")
    try:
